...
| **模拟器** | $\hat{T}(s_{t+1}\,|\,s_t,a_t)$ | 状态(几何/物理) | 结构精确 | 人 + 程序 |
| **规划器** | $\pi(a_t\,|\,s_t,g)$ | 行动序列 | 决策正确 | 智能体 |
...
$$\mathcal{L}_{\text{KL}} = D_{\text{KL}}\big(q_\phi(s_t|h_t,o_t)\;\big\|\;p_\psi(s_t|h_t)\big)$$
...
|------|---------------|----------------|
| **微重力**：$10^{-6}g$ 稳定残差重力 | 地面动力学模型完全失效，但物理规则可精确建模 | RSSM 的 $T(s_{t+1}\,|\,s_t,a_t)$ 需替换为微重力多体动力学 |
