...
其中各变量的含义为：
- $S$：世界状态空间，变量 $s_t$ 属于该空间（$s_t \in S$），为 $t$ 时刻不可直接观测的真实世界状态
- $A$：动作空间，变量 $a_t$ 属于该空间（$a_t \in A$），为智能体采取的行动
- $O$：观测空间，变量 $o_t$ 属于该空间（$o_t \in O$），为智能体接收到的观测
- $T(s_{t+1}\mid s_t,a_t)$：状态转移概率函数
- $E(o_t\mid s_t)$：观测发射概率函数
- $R(s_t,a_t)$：奖励函数
- $\gamma$：折扣因子，$\gamma\in[0,1)$

在此框架下，世界模型被严格定义为对状态转移函数 $T$、观测函数 $E$ 和奖励函数 $R$ 的近似学习。一个仅学习 $p(o_{t+1} \mid o_t, a_t)$ 但不对状态 $s_t$ 做结构化假设的模型，即使自称"世界模拟器"，也不符合这一定义。
...
低轨（Low Earth Orbit, LEO）星载机器人代表了太空具身智能中最具近期可行性的应用场景。与深空探测相比，低轨环境具有相对明确的物理规则——微重力可近似为稳定残差重力（$10^{-6}g$ 量级），热控环境周期可预测（约 90 分钟轨道周期交替日照/阴影），通信延迟可控（毫秒级）。这使得低轨星载机器人成为世界模型从地面迁移到太空的最自然的"第一站"。

#### 6.2.1 低轨环境的独特挑战

| 挑战 | 对世界模型的影响 | 与地面环境的差异 |
|------|---------------|----------------|
| **微重力**：$10^{-6}g$ 稳定残差重力 | 地面动力学模型完全失效，但物理规则可精确建模 | RSSM 的 $T(s_{t+1}\mid s_t,a_t)$ 需替换为微重力多体动力学 |
| **热真空循环**：$\pm150^\circ$C，周期 90 分钟 | 材料热形变、润滑剂挥发影响机械臂精度 | Simulator 需耦合热-结构-动力学多物理场 |
...
- **动力学**：微重力下多刚体动力学方程已知，$\dot{\omega} = I^{-1}(\tau - \omega \times I\omega)$，不确定性主要来自惯量参数 $I$ 和接触模型
